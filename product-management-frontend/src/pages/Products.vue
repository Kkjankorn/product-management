<template>
  <div>
    <h3 class="text-xl font-semibold mb-4 text-gray-100">Products</h3>
    <div class="flex flex-col sm:flex-row gap-2 mb-4 items-center">
      <button @click="openAdd" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">+ Add Product</button>
    ...
    <div class="flex flex-col min-h-[400px]"> <!-- เพิ่ม min-h เพื่อความสูงคงที่ -->
      <ProductTable :products="products" @edit="onEdit" @delete="onDelete" />
      ...
      <div class="flex justify-center items-center gap-4 mt-auto mb-4">
        ...
        <button @click="nextPage" :disabled="products.length < limit" class="px-4 py-2 rounded bg-gray-700 text-gray-100 disabled:opacity-50">ถัดไป</button>
      </div>
    </div>
    ...
    <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div class="bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md relative animate-fadeIn">
        ...
        <ProductForm :categories="categories" :editItem="editing" @save="onSave" @close="closeForm" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      const res = await getProducts(page.value, limit, search.value);
      products.value = res.data || [];
    };

    const loadCategories = async () => {
      const res = await getCategories(1, 5);
      categories.value = res.data || [];
    };

    const openAdd = () => { editing.value = null; showForm.value = true; };
    const closeForm = () => { showForm.value = false; editing.value = null; };
    const onEdit = (p) => { editing.value = p; showForm.value = true; };
    ...
    const onSave = async (payload) => {
      if (editing.value) {
        await updateProduct(editing.value._id, payload);
      } else {
    ...
    return { products, categories, showForm, editing, openAdd, closeForm, onEdit, onDelete, onSave, search, load, page, nextPage, prevPage, limit };
  }
};
</script>
